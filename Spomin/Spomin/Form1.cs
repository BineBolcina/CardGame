...
namespace Spomin
{
    public partial class Form1 : Form
    {
        ...
        private void FlipCard(PictureBox pb, bool isFromAI)
        {
            ...
            else
            {
                if (game.CheckMatch(game.FirstSelected, card))
                {
                    ...
                    // 🔁 če AI ali igralec zadane, naj nadaljuje
                    if (!isFromAI)
                        return; // igralec igra naprej, ne kliči AI-ja
                    else
                        AIMove(); // AI nadaljuje svojo potezo
                }
                else
                {
                    // ❌ neuspešen poskus
        ...
        private void ResetGame()
        {
            ...
            //Premesamo karte
            var shuffledCards = game.Deck.Cards.OrderBy(c => Guid.NewGuid()).ToList();
